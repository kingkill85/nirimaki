...
Item {
    ...
    // Install/Remove filter — read from ~/.cache/nirimaki/state.json,
    // generated by bin/nirimaki-feature-state on session start + after
    // every install/remove. Keys are feature names (matching the
    // visibleWhen.feature field on JSON menu nodes); values are booleans.
    ...
    // True if the node's visibleWhen predicate is satisfied (or absent).
    // Branches recurse: visible iff at least one child is visible.
    ...
    function rebuild() {
        ...
        for (let i = 0; i < kids.length; i++) {
            ...
            const n = tree[id];
            if (!n) continue;
            // Hide leaves whose visibleWhen predicate isn't satisfied,
            // and branches that have no visible descendants.
